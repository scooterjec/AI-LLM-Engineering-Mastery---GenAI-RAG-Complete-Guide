...
class SimpleModelSelector:
    ...
    def __init__(self):
        ...
        self.llm_models = { "ollama": "Llama3","openai": "GPT-4" }
...
class SimpleRAGSystem:
    ...
    def setup_embedding_function(self):
        """Setup the appropiate embedding function"""
        try:
    ...
    def setup_collection(self):
        ...
        collection_name = f"docuemnts_{self.embedding_model}"
    ...
    def get_embedding_info(self):
        """Get embedding model info for display"""
...
def main():
    ...
    # Check if embedding model change
    if embedding_model != st.session_state.current_embedding_model:
        st.session_state.processed_files.clear()  # Reset processed files when embedding model changes
    ...
    try:
        if st.session_state.rag_system is None:
            st.session_state.rag_system = SimpleRAGSystem(embedding_model=embedding_model, llm_model=llm_model)
        ...
        with st.spinner("Processing PDF..."):
            try: 
                ...
                if st.session_state.rag_system.add_documents(chunks):
                    st.success(f"Processed and added {len(chunks)} chunks from {pdf_file.name}")
