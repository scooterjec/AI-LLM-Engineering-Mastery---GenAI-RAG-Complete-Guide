...
# input_file = formatted_custom_support.json ; output_file = output.json
...
def check_file_format(dataset):
    ...
    for ex in dataset:
        ...
        for message in messages:
            ...
            if any(
                    k not in ("role", "content", "name", "function_call") for k in message.keys()
                ):
...
# convert jsonl from json
# we will use a different json dataset maybe from hugginface!!
...
print(f"Datset has ~{n_billing_tokens_in_dataset} tokens that will be charged for durin training.")
print(f"By default, you'll train for {n_epochs} epochs on this dataset.")
print(f"By default you'll be charged for ~{n_billing_tokens_in_dataset * n_epochs} tokens during training.")
...
training_file = clients.files.create(file=open("output.jsonl", "rb"), purpose="fine-tune" )
print(f"File uploaded with id: {training_file.id}")


# training-file.id: file-ddfasdfddw
...
response = client.fine_tuning.jobs.create(
        ...
        model="gpt-4o-2024-08-06", # Se obtiene en https://platform.openai.com/docs/guides/fine-tuning
        hyperparameters = {
                "n_epochs": 5
            }
    )
...
# it's binary, so read it and then make it a file like object
...
print(f"Response from base model: {response.choices[0].message.content}")
print("\n ==========")

## now test the fine-tuned model with the same prompt
...
print(f"Response from fine-tuned model: {response.choices[0].message.content}")
